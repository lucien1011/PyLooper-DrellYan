...
class CSVProducer(Module):
    def analyze(self,data,dataset,cfg):
        arr = np.concatenate(
                [
                    np.expand_dims(cfg.collector.mu_pt[:,0],axis=1),
                    np.expand_dims(cfg.collector.mu_eta[:,0],axis=1),
                    np.expand_dims(cfg.collector.mu_phi[:,0],axis=1),
                    np.expand_dims(cfg.collector.mu_pt[:,1],axis=1),
                    np.expand_dims(cfg.collector.mu_eta[:,1],axis=1),
                    np.expand_dims(cfg.collector.mu_phi[:,1],axis=1),
